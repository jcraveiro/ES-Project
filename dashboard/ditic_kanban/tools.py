...
def user_tickets_details(rt_object, email):
    ...
    if email == 'inbox':
        query = 'Queue = "general" AND "CF.{IS - Informatica e Sistemas}" = "INBOX" AND Owner = "Nobody"  AND ' \
                'Status != "resolved" AND Status != "deleted" '

    # If the user is unknown, then search all users but those that we already know
    elif email == 'unknown':
        query = 'Queue = "general" AND "CF.{IS - Informatica e Sistemas}" LIKE "INBOX%"  AND ' \
                'Status != "resolved" AND Status != "deleted" '
...
def ticket_actions(rt_object, ticket_id, action, ticket_email, user_email):
    ...
    if action == 'increase_priority':
        newPriority = int(ticket_line['priority']) + 1
    ...
    # BACK ##############################
        ...
        if ticket_email == 'inbox':
            result = modify_ticket(
                rt_object,
                ticket_id,
                {
                    'cf.{is - informatica e sistemas}': 'inbox',
                }
            )
    ...
    # URGENT ##############################
        ...
        if int(ticket_line['priority']) < 200:
            priority = 200
        else:
            priority = int(ticket_line['priority'])
...
def search_tickets(rt_object, search):
    ...
    # The search string
    query = 'Queue = "General" AND ( "CF.{IS - Informatica e Sistemas}" = "INBOX" ' \
            'AND ( Lastupdated > "'+previous_date+'" OR Created > "'+previous_date+'") ' \
...
def get_urgent_tickets(rt_object):
    ...
    :return:
    """
    query = '"cf.{is - informatica e sistemas}" = "inbox" AND "cf.{ditic-urgent}" = "yes"' \
            'AND Owner = "nobody"'
